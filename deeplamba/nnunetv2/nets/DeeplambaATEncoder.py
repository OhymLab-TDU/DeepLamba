...
class DeeplambaATEncoder(nn.Module):
    def __init__(
            self,
            in_chans=1,
            out_chans=13,
        ...
        self.patchembed = PatchEmbed2D()  # first downsample
        ...
        self.endconv = nn.Conv2d(in_channels=384, out_channels=self.out_chans, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m1, feat_m = self.SeAt1(x)
        x = feat_m.view(B, H, W, C)
        seg_m1 = seg_m1.view(B, H, W, C)
        enc2 = seg_m1.permute(0, 3, 2, 1).contiguous()
        x = self.patchmerge[0](x)
        for vssblock2 in self.vssBlock2:
            x = vssblock2(x)
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m2, feat_m = self.SeAt2(x)
        x = feat_m.view(B, H, W, C)
        seg_m2 = seg_m2.view(B, H, W, C)
        enc3 = seg_m2.permute(0, 3, 2, 1).contiguous()
        # enc3 = self.upblock2(enc3)
        enc3 = self.patchexpand2(enc3)
        enc3 = enc3.permute(0, 3, 2, 1).contiguous()
        x = self.patchmerge[1](x)
        for vssblock3 in self.vssBlock3:
            x = vssblock3(x)
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m3, feat_m = self.SeAt3(x)
        x = feat_m.view(B, H, W, C)
        seg_m3 = seg_m3.view(B, H, W, C)
        enc4 = seg_m3.permute(0, 3, 2, 1).contiguous()
        # enc4 = self.upblock3(enc4)
        enc4 = self.patchexpand3(enc4)
        enc4 = enc4.permute(0, 3, 2, 1).contiguous()
        ...
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m4, feat_m = self.SeAt4(x)
        x = feat_m.view(B, H, W, C)
        seg_m4 = seg_m4.view(B, H, W, C)
        enc5 = seg_m4.permute(0, 3, 2, 1).contiguous()
        enc5 = self.patchexpand4(enc5)
        enc5 = enc5.permute(0, 3, 2, 1).contiguous()
        enc5 = self.patchexpand3(enc5)
        enc5 = enc5.permute(0, 3, 2, 1).contiguous()
        enc5 = self.patchexpand2(enc5)
        enc5 = enc5.permute(0, 3, 2, 1).contiguous()
        out = torch.cat([enc2, enc3, enc4, enc5], dim=1)
        out = self.finalpatchexpand(out)
...
def get_DeeplambaAT_from_plans(
...
if __name__ == '__main__':
    ...
    model = DeeplambaATEncoder(in_chans=3, out_chans=13, feat_size=[48, 96, 192, 384, 768], depths=[2, 2, 9, 2],
                        drop_path_rate=0.2)
