...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        x = input + self.drop_path(self.Se(self.self_attention(self.ln_1(input))))
        return x
...
class DeepSeMamba(nn.Module):
    ...
    def forward(self, x):
        ...
        enc2 = x.permute(0, 3, 1, 2).contiguous()  # 1,96,128,128
...
def get_DeepSeMamba_from_plans(
...
if __name__ == '__main__':
    ...
    # model = DeepSeMamba(in_chans=3, out_chans=13, feat_size=[48, 96, 192, 384, 768], depths=[2, 2, 9, 2],
    #                     drop_path_rate=0.2)
    # model = SemanticAttention(dim=96, n_cls=13) # 4, 512 ,96 ----> 4, 512, 13
    total_params = sum(p.numel() for p in model.parameters())
    print(f'Total parameters: {total_params}')
    x = torch.rand(4, 512, 512, 96)
