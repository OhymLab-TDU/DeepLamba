...
class DeeplambaAT(nn.Module):
    def __init__(
        ...
        self.SeB4 = SE_Block(feat_size[3])
    ...
    def forward(self, x):
        ...
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m, feat_m = self.SeAt1(x)
        x = feat_m.view(B, H, W, C)
        enc2 = x.permute(0, 3, 2, 1).contiguous()
        x = self.patchmerge[0](x)
        for vssblock2 in self.vssBlock2:
            x = vssblock2(x)
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m, feat_m = self.SeAt2(x)
        x = feat_m.view(B, H, W, C)
        enc3 = x.permute(0, 3, 2, 1).contiguous()
        enc3 = self.SeB3(enc3)
        ...
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m, feat_m = self.SeAt3(x)
        x = feat_m.view(B, H, W, C)
        enc4 = x.permute(0, 3, 2, 1).contiguous()
        enc4 = self.SeB4(enc4)
        ...
        B, H, W, C = x.shape
        x = x.view(B, H * W, C)
        seg_m, feat_m = self.SeAt4(x)
        x = feat_m.view(B, H, W, C)
        enc5 = x.permute(0, 3, 2, 1).contiguous()
        enc5 = self.SeB5(enc5)
...
def load_pretrained_ckpt(
    ...
    layers = ['patchexpand4', 'patchexpand3', 'patchexpand2', 'finalpatchexpand', 'endconv']
    params = ['expand.weight', 'norm.weight', 'norm.bias']
    skip_params = [f"{layer}.{param}" for layer in layers for param in params]
...
if __name__ == '__main__':
    ...
    model = DeeplambaAT(in_chans=3, out_chans=13, feat_size=[48, 96, 192, 384, 768], depths=[2, 2, 9, 2],
                        drop_path_rate=0.2)
