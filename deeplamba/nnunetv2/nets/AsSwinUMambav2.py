...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[9, 9],
                 dims=[96, 192], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
...
class AS_SwinUMambav2(nn.Module):
    def __init__(
            self,
            in_chans=1,
            out_chans=13,
        ...
        self.SeB1 = SE_Block(feat_size[0])
        self.SeB2 = SE_Block(feat_size[1])
        self.SeB3 = SE_Block(feat_size[2])
        #       self.aspp5 = _ASPP(feat_size[4], feat_size[4], [4, 6, 18], norm_layer=nn.BatchNorm2d)
        self.brfb4 = BasicRFB(feat_size[3], feat_size[3])
        #        self.SeB5 = SE_Block(feat_size[4])
        ...
        self.decoder1 = nn.Sequential(
            nn.Conv2d(feat_size[1] , feat_size[1], kernel_size=1, stride=1, padding=0),
            nn.InstanceNorm2d(feat_size[1], eps=1e-5, affine=True),
        )
    ...
    def forward(self, x_in):
        ...
        enc1 = self.encoder1(x_in)  #B,48,224,224
        enc11 = self.SeB1(enc1)
        enc2 = self.encoder2(vss_outs[0])  #B,96,112,112
        enc22 = self.SeB2(enc2)
        enc3 = self.encoder3(vss_outs[1])  #B,192,56,56
        enc33 = self.SeB3(enc3)
        enc4 = self.encoder4(vss_outs[2])  #B,384, 28,28
        brf4 = self.brfb4(enc4)

        dec1 = self.decoder1(enc22)
        dec2 = self.decoder2(brf4, dec1)
        dec3 = self.decoder3(dec2, enc11)
        dec_out = self.decoder4(dec3)
...
def get_Asswin_umamba_from_plans(
...
#output torch.Size([3, 13, 224, 224])
