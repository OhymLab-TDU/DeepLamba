...
class DeeplambaE(nn.Module):
    def __init__(
        ...
        self._ASPP = _ASPP(in_channels=288, out_channels=288, atrous_rates=(6, 12, 18), norm_layer=nn.BatchNorm2d,
                           norm_kwargs=None)
    ...
    def forward(self, x):
        # x1 = self.stem(x)
        x = self.patchembed(x)  # 1,128,128,96
        for vssblock1 in self.vssBlock1:
            x = vssblock1(x)  #1,128,128,96
        x = x.permute(0, 3, 1, 2).contiguous()  # 1,96,128,128
        enc2 = self.SeB2(x)
        x = enc2.permute(0, 2, 3, 1).contiguous()

        x = self.patchmerge[0](x)  # 1,64,64,192
        for vssblock2 in self.vssBlock2:
            x = vssblock2(x)
        x = x.permute(0, 3, 1, 2).contiguous()
        enc3 = self.SeB3(x)
        x = enc3.permute(0, 2, 3, 1).contiguous()
        ...
        x = x.permute(0, 3, 1, 2).contiguous()
        enc4 = self.SeB4(x)
        x = enc4.permute(0, 2, 3, 1).contiguous()
        ...
        enc_total = self._ASPP(enc_total)
...
if __name__ == '__main__':
    ...
    model = DeeplambaE(in_chans=3, out_chans=13, feat_size=[48, 96, 192, 384, 768], depths=[2, 2, 9, 2],
                       drop_path_rate=0.2)
